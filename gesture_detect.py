...
# Maps bones to a matplotlib color name.   REMOVE
  ...
  for idx in range(num_instances):
    ...
    for edge_pair, color in KEYPOINT_EDGE_INDS_TO_COLOR.items():         #REMOVE later
      if (kpts_scores[edge_pair[0]] > keypoint_threshold and
        ...
        #add to edges to draw + add color to list (order tells it which is which color)
...
def draw_prediction_on_image(  
  ...
  fig = Figure(figsize=(12 * aspect_ratio, 12))
  ...
  if crop_region is not None:        #REMOVE
  ...
  image_from_plot = np.asarray(canvas.buffer_rgba())
  #image_from_plot = image_from_plot.reshape(fig.canvas.get_width_height()[::-1] + (4,))
  image_from_plot = image_from_plot[:, :, :3]  # drop the alpha channel to get RGB
  image_from_plot = image_from_plot.reshape(
      fig.canvas.get_width_height()[::-1] + (3,))
  #plt.close(fig)
  if output_image_height is not None:      #REMOVE
...
def to_gif(images, duration):  #REMOVE
  """Converts image sequence (4D numpy array) to gif."""
  imageio.mimsave('./animation.gif', images, duration=duration)
  return embed.embed_file('./animation.gif')

def progress(value, max=100):  #REMOVE
  return HTML("""
      <progress
          value='{value}'
          max='{max}',
...
if "tflite" in model_name:   #REMOVE
  ...
  if "movenet_lightning" in model_name:
    ...
    module = tf.saved_model.load("./movenet_lightning_local")
    input_size = 192
  ...
  def movenet(input_image):
    ...
    print(f"MoveNet inference time: {end_time - start_time:.4f} seconds")
